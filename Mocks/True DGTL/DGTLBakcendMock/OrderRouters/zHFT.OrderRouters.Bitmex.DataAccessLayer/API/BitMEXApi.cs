...
namespace zHFT.OrderRouters.Bitmex.DataAccessLayer.API
{
    public class BitMEXApi
    {
        ...
        public BitMEXApi(string domain = "https://testnet.bitmex.com", string bitmexKey = "", string bitmexSecret = "", int rateLimit = 5000)
        {
            this.domain = domain;
        ...
        {
            return ((Int64)(DateTime.Now.AddHours(10).Subtract(new DateTime(1970, 1, 1))).TotalSeconds);
        ...
        public string Delete(string function, Dictionary<string, string> param = null, bool auth = false, bool json = false)
        {
            string paramData = json ? BuildJSON(param) : BuildQueryData(param);
            ...
            {
                string expires = GetExpires().ToString();
                string message = "DELETE" + url + expires + postData;
                byte[] signatureBytes = hmacsha256(Encoding.UTF8.GetBytes(apiSecret), Encoding.UTF8.GetBytes(message));
                string signatureString = ByteArrayToString(signatureBytes);
